...
- MQTT-сущности повторно публикуют discovery и retained-статус `online` после восстановления соединения с брокером.
- Периодическая проверка домофона принудительно обновляет availability и исправляет оставшийся в брокере LWT `offline`.
- Все управляющие сущности теперь удерживаются приложением на протяжении всего времени работы, поэтому их MQTT-клиенты не удаляются сборщиком мусора.

## 3.0.37 - 2026-08-14

### Исправления и улучшения

- MQTT-параметр `ssl` теперь действительно включает TLS.
- Добавлены проверка доступности устройств, корректный статус соединения и восстановление SDK-сессии без повторной инициализации всего MQTT-слоя.
- Параметр автоматического снимка добавлен в редактор конфигурации, а интервалы, порты и обязательные поля валидируются до запуска.
- Служебные и редко используемые MQTT-элементы перенесены в подходящие категории или отключены по умолчанию.
- ISAPI-ввод сохраняет тело XML/JSON с пробелами, ограничен методами `GET`/`POST`/`PUT` и путями `/ISAPI/`; произвольный вызов методов через STDIN удалён.
- Исправлены длина URL в SDK-запросах, обработка неизвестного типа устройства и освобождение alarm/login-сессий при завершении.
- Цветовые ANSI-коды отключены в журнале Home Assistant.

## 3.0.36 - 2026-08-14

### Улучшения

- Добавлены русские названия и пояснения для всех параметров приложения.
- Пароли домофона и MQTT теперь скрыты в редакторе конфигурации.
- Обновлены логотип, описание приложения и документация форка.
- Из репозитория удалена отдельная beta-карточка приложения.
